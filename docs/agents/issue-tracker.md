...
- **List issues**: `gh issue list --state open --json number,title,labels --jq '[.[] | {number, title, labels: [.labels[].name]}]'` (fetch body/comments with `gh issue view <number> ...` when needed)
