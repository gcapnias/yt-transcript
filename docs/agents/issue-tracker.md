...
**Version:** `br` 0.6.0. Every command below was verified against this version.

**Workspace:** `.beads/` at the repo root; issue ID prefix `ytdlp` (issue IDs look like `ytdlp-1`).
...
# Re-initialise only if .beads/ is missing (idempotent — safe to re-run)
br init
```
...
- **Comment on an issue**: write the comment text to a file first, then `br comments add --actor "$ACTOR" {issue-id} --file {path} --json`. **Never** pass multi-line or markdown-bearing text via `--message`/`-m` with an inline heredoc (e.g. `--message "$(cat <<'HEREDOC' ... HEREDOC)"`) — a literal `"` anywhere in the content silently truncates/corrupts everything from that character onward with no error, since the heredoc's captured output re-enters shell quoting when substituted into the double-quoted `--message` argument. `--message` is fine only for single-line text with no quote characters.
- **Update an issue**: `br update --actor "$ACTOR" {issue-id} --status <open|in_progress> --priority 1 --add-label triage-reviewed --claim --json` (`--claim` is shorthand for claim-and-start). Terminal states are refused here — use `br close` / `br delete`.
...
- **Sync is explicit**: `br sync --flush-only` exports the database to `.beads/issues.jsonl`. This repo is **not currently under git**. If it is later initialised, run `br sync --flush-only && git add .beads/` before any commit that touches bead state — `br` never runs git commands for you.
- **Triage label vocabulary** is defined in `docs/agents/triage-labels.md`
